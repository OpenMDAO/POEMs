...
When creating an OpenMDAO problem that require models whose outputs are needed as inputs for other models, it's convenient to break down that problem into subproblems, but OpenMDAO does not currently have a component that does this.
...
class SubproblemComp(om.ExplicitComponent):
    ...
    def __init__(self, model=None, driver=None, comm=None, name=None,
        ...
        # get rid of p since it's not being used anymore
        del p
